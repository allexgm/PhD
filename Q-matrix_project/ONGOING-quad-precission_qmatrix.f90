...
! in a new way more efficient. (Quad-precission code)***ONGOING***                 |
...
! The whole code was computed in double precision (kind=16) and the Q-matrix values
...
implicit none
real(kind=16) cms(3), alpha, beta, eta, p, position_A(3), position_B(3), A, B, ka, kb, nm
integer i, j, k, max_i, max_j, max_k, N, min_N, max_N, low_n, m, max_m
real(kind=16) comb, dawson_cheb
real(kind=16), parameter :: pi = 3.14159265358979323846264338327950288419716939937510582097494459230q0
real(kind=16), allocatable :: Xp(:), Xm(:), H(:), F(:), Ga(:), Gb(:), Q(:,:,:), out_Q(:,:,:)
...
position_A = (/0.q0, 0.q0, 0.q0/)
position_B = (/0.q0, 0.q0, 4.0q0/)
cms = (/0.q0, 0.q0, 2.0q0/)
...
max_i = 8 !i: from 0 to 'max_l' for atom A
max_j = 8 !j: from 0 to 'max_l' for atom B
max_k = 2 !k: (in the paper)--> from 2 to 5/6 
...
do N=0, max_N
  !...apply eq.(36) 
  Xp(N) = ((beta*B+alpha*A)/p)**N * exp(p*((beta*B+alpha*A)/p)**2.q0) + &
          ((beta*B-alpha*A)/p)**N * exp(p*((beta*B-alpha*A)/p)**2.q0)
  Xm(N) = ((beta*B+alpha*A)/p)**N * exp(p*((beta*B+alpha*A)/p)**2.q0) - &
          ((beta*B-alpha*A)/p)**N * exp(p*((beta*B-alpha*A)/p)**2.q0)  
enddo
...
do N=max_N, min_N, -1
  ...
  if (mod(N, 2) == 0.and.N.ge.2) then
    ...
    max_m = low_n - 1
    do m=0, max_m 
      nm = real(low_n, kind=16) - real(m, kind=16) - 0.5q0
      !... apply eq.(34) 
      H(N) = H(N) + 0.25q0 * comb(2*low_n-2,2*m) * p**(-nm) * gamma(nm) * Xp(2*m)
      F(N) = F(N) + 0.25q0 * comb(2*low_n-2,2*m) * p**(-nm) * gamma(nm) * Xm(2*m)
    enddo
  !2)-if N > or = 2 and odd...
    ...
    max_m = low_n - 1
    do m=0, max_m
      nm = (real(N-1, kind=16))/2 - real(m, kind=16) - 0.5q0
      !... apply eq.(35)
      Gb(2*low_n+1) = Gb(2*low_n+1) + 0.25q0 * comb(2*low_n-1,2*m+1) * p**(-nm) * gamma(nm) * Xm(2*m+1)
    enddo
  !3)-if N < 2 and odd...
  else if (mod(N, 2) /= 0.and.N.lt.2) then
    if (N==1) then
    !... if N=1 apply eq.(40)...
      Gb(1) = 0.5q0 * sqrt(pi) * (exp(p*((beta*B+alpha*A)/p)**2q0)*dawson_cheb(sqrt(p)*((beta*B+alpha*A)/p))- &
                                  exp(p*((beta*B-alpha*A)/p)**2q0)*dawson_cheb(sqrt(p)*((beta*B-alpha*A)/p)))               
      Ga(1) = 0.5q0 * sqrt(pi) * (exp(p*((alpha*A+beta*B)/p)**2q0)*dawson_cheb(sqrt(p)*((alpha*A+beta*B)/p))- &
                                  exp(p*((alpha*A-beta*B)/p)**2q0)*dawson_cheb(sqrt(p)*((alpha*A-beta*B)/p)))
    else
    !... otherwise apply eq.(38)
      Gb(N) = 2.q0*p/(N-1)*Gb(N+2)-ka/(N-1)*H(N+1)-kb/(N-1)*F(N+1)
      Ga(N) = 2.q0*p/(N-1)*Ga(N+2)-kb/(N-1)*H(N+1)-ka/(N-1)*F(N+1)
    endif
  endif     
  !4)-if N < 2 and even...
  if (mod(N, 2) == 0.and.N.lt.2) then 
  !... apply eq.(37) and eq.(39)
    F(N) = 2.q0*p/(N-1)*F(N+2)-ka/(N-1)*Ga(N+1)-kb/(N-1)*Gb(N+1)
    H(N) = 2.q0*p/(N-1)*H(N+2)-kb/(N-1)*Ga(N+1)-ka/(N-1)*Gb(N+1)
  endif
enddo
...
do N=min_N, max_N
  Q(0,1,N) = 1.q0/(ka*kb)*(1.q0*Gb(N)-1/kb*F(N-1))
enddo
!For Q0jk apply eq.(29), but j=0 and j=1 were already calculated (start in j=2)
do j=2, max_j
  do k=min_N, max_N
    if (mod(j+k,2) /= 0) cycle
    Q(0,j,k) = 1.q0*Q(0,j-2,k)-(2*j-1)/(2.q0*beta*B)*Q(0,j-1,k-1) 
    if(Q(0,j,k) < 1.d-12) Q(0,j,k) = 0.q0
  enddo
enddo
...
do i=1, max_i
  do j=i, max_j
    do k=min_N, max_N
      ...
      Q(i,j,k) = real(2+j-i-k, kind=16)/(2.q0*alpha*A)*Q(i-1,j,k-1)-beta*B/(alpha*A)*Q(i-1,j-1,k)+p/(alpha*A)*Q(i-1,j,k+1)
      if(Q(i,j,k) < 1.d-12) Q(i,j,k) = 0.q0
    enddo
  enddo
enddo
...
real(kind=16) function comb(a,b)
    implicit none
    integer, intent(in) :: a,b
    !factorial(x)=gamma(x+1)
    comb = gamma(real(a+1, kind=16)) / (gamma(real(b+1, kind=16))*gamma(real(a-b+1, kind=16)))
end function comb
...
function dawson_cheb(x) result(dawson_result)
  ...
  real(kind=16) :: dawson_result
  double precision :: y, sum_p, sum_q, frac, w2
  integer :: i

  double precision, parameter :: zero = 0.0q0
  double precision, parameter :: half = 0.5q0
  double precision, parameter :: one = 1.0q0
  double precision, parameter :: six25 = 6.25q0
  double precision, parameter :: one225 = 12.25q0
  double precision, parameter :: two5 = 25.0q0
