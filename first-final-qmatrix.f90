...
!
!----------------------------------------------------------------------------------+
! Code to compute the Q-matrix needed for the ECPs in a new way more efficient.    |
...
max_k = 7 !k: (in the paper)--> from 2 to 5/6 
...
do N=max_N, min_N, -1
  ...
  !3)-if N < 2 and odd...
    ...
    if (N==1) then
    !... if N=1 apply eq.(40)...
      Gb(1) = 0.5d0 * sqrt(pi) * (exp(p*((beta*B+alpha*A)/p)**2d0*dawson_cheb(sqrt(p)*p*((beta*B-alpha*A)/p))))
      Ga(1) = 0.5d0 * sqrt(pi) * (exp(p*((alpha*A+beta*B)/p)**2d0*dawson_cheb(sqrt(p)*p*((alpha*A-beta*B)/p))))
      write(6,*) sqrt(p)*p*((alpha*A-beta*B)/p), sqrt(p)*p*((beta*B-alpha*A)/p)
    else
...
!COMPARING WAYS TO CALCULATE DAWSON FUNCTION:
!open(unit=11, file='dawson-fun.dat', action='write')
!write(11,*) '# x, dawson_euler(x),  dawson_cheb(x)'
!do i=1, 10000
!    x = i/10d0
!    write(11,*) x, dawson_euler(x), dawson_cheb(x)
!enddo
...
!Define Dawson function with Taylor series:
!See https://en.wikipedia.org/wiki/Dawson_function 
real(kind=8) function dawson_taylor(x)
    implicit none
    ...
    term = x
    n = 0
    ! Taylor series for Dawson's function
    do while (abs(term) > 1.0e-10)
        sum = sum + term 
        n = n + 1
        term = (-2.0 * x**2 * term) / (2.0 * n + 1.0)
        !another possible way --> term = ((-2)**n*x**(2*n+1))/double_factorial(2*n+1)
    end do
    dawson_taylor = sum
end function dawson_taylor

!Define Dawson function with Euler method:
real(kind=8) function dawson_euler(x)
    implicit none
    real(kind=8), intent(in) :: x
    real(kind=8) :: h, xi, fx
    integer :: n, i
